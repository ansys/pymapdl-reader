...
template <class T> char *ReadBsparseRecord(T *buffer, int *size) {
  int *raw = (int *)buffer;
  *size = *raw++;
  int bitcod = *raw++;
...
template <class T> void ReadBsparseRecordToVec(int *raw, int *size, T *vec) {
  *size = *raw++;
  int bitcod = *raw++;
...
template <class T> char *ReadWindowedSparseBuffer(T *buffer, int *size) {
  ...
  *size = *raw++;
  ...
  if (NWin > 0)
    do {
      ...
      if (iLoc > 0)  /* ===== One isolated NonZero Value - no need to store a
                        Window Len */
      {
        vec[iLoc] = ((T *)raw)[0];
        raw += iShift;
  ...
  *size = *raw++;
  ...
  if (NWin > 0)
    do {
      ...
      {
        double *adr =
            vec + (-iLoc); /* Start of the Windows in the output vector */
        iLen = *raw++;     /* Length of the Window */
...
void *read_record(const char *filename, int64_t ptr, int *prec_flag,
                  int *type_flag, int *size, int *out_bufsize) {

  int bsparse_flag, wsparse_flag, zlib_flag;

  ifstream binFile(filename, ios::in | ios::binary);
  binFile.seekg(ptr * 4);
  int bufsize = read_header(&binFile, &bsparse_flag, &wsparse_flag, &zlib_flag,
                            prec_flag, type_flag);
...
// This function differs from read_record as it must be supplied with ``arr``,
// which must be sized properly to support the data coming from the file.
void read_record_stream(ifstream *file, int64_t loc, void *arr, int *prec_flag,
  ...
  int bsparse_flag, wsparse_flag, zlib_flag;
  int bufsize = read_header(file, &bsparse_flag, &wsparse_flag, &zlib_flag,
                            prec_flag, type_flag);
  *size = bufsize;
...
void *read_record_fid(fstream *file, int64_t loc, int *prec_flag,
                      int *type_flag, int *size, int *out_bufsize) {
...
void read_nodes(const char *filename, int64_t ptrLOC, int nrec, int *nnum,
  ...
  for (n = 0; n < nrec; n++) {
    read_record_stream(&binFile, -1, raw, &prec_flag, &type_flag, &bufsize);
...
int overwriteRecord(fstream *fs, int ptr, double *data) {
  ...
  // seek to data location if supplied with a pointer
  fs->seekg(ptr * 4);
  int bufsize = read_header(fs, &bsparse_flag, &wsparse_flag, &zlib_flag,
                            &prec_flag, &type_flag);
  ...
  if (type_flag) {   // int16 or int32
    ...
    if (prec_flag) { // float
      ...
      // fs->write(reinterpret_cast<const char *>(&float_array),
