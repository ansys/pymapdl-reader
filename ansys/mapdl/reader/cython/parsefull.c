...
// To Optimize
// - read in blocks rather than integers

// To add
// - add big endianness compatibility
...
// returns an integer from a position along a char array
...
void read_full(int *numdat, int *nref, int *dref, int *krows, int *kcols,
  ...
  kentry = read_array(&krows, &kcols, &kdata, rref, cref, isfree, ntermK, neqn,
                      ptr, ptrSTF, skipped);

  // Read mass matrix
  mentry = read_array(&mrows, &mcols, &mdata, rref, cref, isfree, ntermM, neqn,
                      ptr, ptrMAS, skipped);
